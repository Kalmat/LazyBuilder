...
public class ProtectBlockContainer extends Container {
	...
	public ItemStack transferStackInSlot(final PlayerEntity player, final int index) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			boolean allowed = item.equals(Items.DIAMOND) || item.equals(Items.EMERALD);
			final int containerSlots = Refs.protectContainerRows * Refs.containerCols;
			...
			if (index < containerSlots) {
				if (!this.mergeItemStack(itemstack1, containerSlots, this.inventorySlots.size(), true)) {
					...
					if (player.world.isRemote) {
						LazyBuilderClient.setTextActive(Refs.onlyItemsProtectMsg, 200);
					}
					return ItemStack.EMPTY;
				}
	...
	public ItemStack slotClick(int slotId, int dragType, ClickType clickTypeIn, PlayerEntity player) {
		...
		if (slotId >= containerSlots && slotId < this.inventorySlots.size()) {
			...
			boolean allowed = !slot.getHasStack() || (slot.getHasStack() && (item.equals(Items.DIAMOND) || item.equals(Items.EMERALD)));
